...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.multiprocessing:
        ...
        results, distance_list, tardiness_list = _eval_dataset(model, dataset, width, softmax_temp, opts, device)
    ...
    print(f"Average distance: {np.mean(distance_list)} tardiness: {np.mean(tardiness_list)}")
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                # This returns (batch_size, iter_rep shape)
                    sequences, costs, distance, tardiness = model.sample_many(batch, batch_rep=batch_rep, iter_rep=iter_rep)
                else:
                    with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                        sequences, costs, distance, tardiness = model.sample_many(batch, batch_rep=batch_rep, iter_rep=iter_rep)

                if torch.is_tensor(distance):
                    distance_list.extend(distance.detach().cpu().reshape(-1).tolist())
                else:
                    distance_list.append(float(distance))
                if torch.is_tensor(tardiness):
                    tardiness_list.extend(tardiness.detach().cpu().reshape(-1).tolist())
                else:
                    tardiness_list.append(float(tardiness))
        ...
        # print(f"distance:{distance} tardiness:{tardiness}")
