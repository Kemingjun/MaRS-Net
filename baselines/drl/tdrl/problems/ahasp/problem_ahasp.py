...
def _resolve_instance_path(filename):
    ...
    for parent in Path(__file__).resolve().parents:
        candidate = parent / "Instance" / filename
        if candidate.exists():
            return candidate
    return Path(__file__).resolve().parent.parent.parent / "Instance" / filename


class AHASP(object):

    NAME = 'ahasp'  # heterogeneous robot scheduling problem
        ...
        if filename is not None:
            if os.path.splitext(filename)[1] == '.pkl':
                ...
                if os.path.splitext(filename)[1] == '.xlsx':
                    ...
                    file_name = _resolve_instance_path(filename)
                    ...
                    base_dir = _resolve_instance_path(filename)
            ...
            # paramet_ahasp.deadline_norm_min = ddl_base
            # paramet_ahasp.deadline_norm_max = ddl_base + 40 * (size - 1)

            # paramet_ahasp.current_time_norm = paramet_ahasp.deadline_norm_max
            ...
            source = torch.stack([source_x, source_y], dim=-1)  # shape: (num_samples, size, 2)
            ...
            # paramet_ahasp.location_norm = x_bound[1]

            
            operation_time = 60 + 5 * torch.randint(0, 9, (num_samples, size), dtype=torch.int32)

            # paramet_ahasp.operation_norm_min = 60
            # paramet_ahasp.operation_norm_max = 60 * 5 + 8
