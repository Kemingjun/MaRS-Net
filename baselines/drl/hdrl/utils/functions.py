...
def sample_many(inner_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        ...
        distance_list.append(distance.view(batch_rep, -1).t())
        tardiness_list.append(tardiness.view(batch_rep, -1).t())
    ...
    min_distance = distance_list[0][torch.arange(pis.size(0), out=argmincosts.new()), argmincosts]
    min_tardiness = tardiness_list[0][torch.arange(pis.size(0), out=argmincosts.new()), argmincosts]

    return minpis, mincosts, min_distance, min_tardiness
