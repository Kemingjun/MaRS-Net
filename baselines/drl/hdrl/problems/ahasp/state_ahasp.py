...
class StateAHASP(NamedTuple):
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        route = torch.full(
            (batch_size, paramet_ahasp.ROBOT_NUM, n_loc),
            -1, dtype=torch.long, device=source_coords.device
        )

        route_len = torch.zeros(batch_size, paramet_ahasp.ROBOT_NUM, dtype=torch.long, device=source_coords.device)
