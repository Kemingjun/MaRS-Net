...
The Gantt charts in Figure 2 provide qualitative insight into the scheduling structures generated by ALNS and MaRS-Net for an instance with `n = 40`. MaRS-Net achieves a substantially lower objective value (`3490.70` vs. `4833.53`), mainly due to a significant reduction in tardiness (`1747.69` vs. `4164.53`), while maintaining a comparable travel distance (`6105.21` vs. `5837.05`). The schedule produced by MaRS-Net is more compact and better synchronized: its CR synchronization waiting time is reduced from `897.84 s` to `194.30 s`, and its total WR idle time is reduced from `8051.41 s` to `5668.68 s`. Accordingly, MaRS-Net achieves a higher WR utilization rate (`62.06%` vs. `53.26%`), indicating more effective coordination between carrier and worker operations under synchronization constraints.
...
MaRS-Net/
  ...
  docs/
    ...
    MaRS-Net_Supplementary_Material.pdf # Supplementary analyses
```
...
```bash
git lfs install
git clone https://github.com/Kemingjun/MaRS-Net.git
cd MaRS-Net
git lfs pull
```
...
More commands are provided in [docs/reproduce.md](docs/reproduce.md). Supplementary analyses, including the MILP formulation, training cost, seed sensitivity, time-quality tradeoff, and sensitivity studies, are provided in [docs/MaRS-Net_Supplementary_Material.pdf](docs/MaRS-Net_Supplementary_Material.pdf).
